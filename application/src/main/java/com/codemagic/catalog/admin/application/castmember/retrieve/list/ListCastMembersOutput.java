...
public record ListCastMembersOutput(String name, CastMemberType type, Instant createdAt) {
    public static ListCastMembersOutput from(final CastMember member) {
        return new ListCastMembersOutput(member.getName(), member.getType(), member.getCreatedAt());
    }
}
